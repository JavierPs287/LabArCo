...
// Paralelizar el bucle principal de bloques con OpenMP
#pragma omp parallel for private(block_ad_map) 
for(; blockIndex < nBlocks; blockIndex++){
  ANNOTATE_ITERATION_TASK("for body of third stage");

  // Crear el vector de bloque, este bucle también puede ser paralelo si es necesario
  ...
  // Llamada a la función que se asume es independiente entre bloques
